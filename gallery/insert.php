...
<?
    ...
    //새글삽입  $table , 폼입력값 , 세션변수
		//수정삽입 $table, 폼 입력 값, 세션변수, $num, $mode, $page
	...
	if ($mode=="modify") // 수정이면
	{
		$num_checked = count($_POST['del_file']);  //삭제 체크 개수
		$position = $_POST['del_file'];            

		for($i=0; $i<$num_checked; $i++)                      // delete checked item
		{
			$index = $position[$i];  // 0 1 2
			$del_ok[$index] = "y";  //$del_ok = ['y','y','y'];
		}
		...
		for ($i=0; $i<$count; $i++)					// update DB with the value of file input box
		{
			...
			$org_real_value = $copied_file_name[$i]; // 2022_02_02_12_43_15_0.jpg
			if ($del_ok[$i] == "y")
			{
